...
def makeGenMETArray(tree, what, etaCut):
    ...
    for i in xrange(tree.GetEntries()):
        ...
        for j in xrange(getattr(tree, "nGenPart")):
            if ( abs(Id[j])==12 or abs(Id[j])==14 or abs(Id[j])==16 ) and pt[j] > 5:
                num+=1
        for j in xrange(getattr(tree, "nGenPart")):
            if (num != 4): continue
            if ( abs(Id[j])==12 or abs(Id[j])==14 or abs(Id[j])==16 ) and pt[j] > 5:
...
#aGenMET, aPFMET, aPuppiMET, aCorrPFMET, aCorrPuppiMET
      ...
      for objset,things in whats:
          ...
          if options.what_reg:
              if not any(re.match(p+"$",objset) for p in options.what_reg.split(",")): 
                  continue
          plots = []
          froot = TFile('VBF_stuff.root', 'recreate')
          tree = TTree('Events', 'simple tree')
          for name,obj,col,msty,msiz in things:
              if "GenAcc$" in obj: continue
              obj = obj.replace("$","")
              if obj=="Gen":
                  #aMET, aMETphi, a1, a2 = makeGenMETArray(signal, obj, options.eta)
                  aGenMET, aGenMETphi, aGenNu1, aGenNu2, aGenNu3, aGenNu4 = makeGenMETArray(signal, obj, options.eta)
                  nGenEvent = 0
                  tree.Branch('nGenEvent', nGenEvent, 'nGenEvent/I')
                  tree.Branch('GenMET', aGenMET, 'aGenMET[nGenEvent]/F')
                  tree.Branch('GenMET_phi', aGenMET_phi, 'aGenMET_phi[nGenEvent]/F')

#                  tree.Branch('GenNu1_pt', aGenNu1_pt, 'aGenNu1_pt[nGenEvent]/F')
#                  tree.Branch('GenNu2_pt', aGenNu2_pt, 'aGenNu2_pt[nGenEvent]/F')
#                  tree.Branch('GenNu3_pt', aGenNu3_pt, 'aGenNu3_pt[nGenEvent]/F')
#                  tree.Branch('GenNu4_pt', aGenNu4_pt, 'aGenNu4_pt[nGenEvent]/F')
#                  tree.Branch('GenNu1_eta', aGenNu1_eta, 'aGenNu1_eta[nGenEvent]/F')
#                  tree.Branch('GenNu2_eta', aGenNu2_eta, 'aGenNu2_eta[nGenEvent]/F')
#                  tree.Branch('GenNu3_eta', aGenNu3_eta, 'aGenNu3_eta[nGenEvent]/F')
#                  tree.Branch('GenNu4_eta', aGenNu4_eta, 'aGenNu4_eta[nGenEvent]/F')
#                  tree.Branch('GenNu1_phi', aGenNu1_phi, 'aGenNu1_phi[nGenEvent]/F')
#                  tree.Branch('GenNu2_phi', aGenNu2_phi, 'aGenNu2_phi[nGenEvent]/F')
#                  tree.Branch('GenNu3_phi', aGenNu3_phi, 'aGenNu3_phi[nGenEvent]/F')
#                  tree.Branch('GenNu4_phi', aGenNu4_phi, 'aGenNu4_phi[nGenEvent]/F')
#                  tree.Branch('GenNu1_mass', aGenNu1_mass, 'aGenNu1_mass[nGenEvent]/F')
#                  tree.Branch('GenNu2_mass', aGenNu2_mass, 'aGenNu2_mass[nGenEvent]/F')
#                  tree.Branch('GenNu3_mass', aGenNu3_mass, 'aGenNu3_mass[nGenEvent]/F')
#                  tree.Branch('GenNu4_mass', aGenNu4_mass, 'aGenNu4_mass[nGenEvent]/F')
                  tree.Branch('GenNu1_pt', aGenNu1[0], 'aGenNu1_pt[nGenEvent]/F')
                  tree.Branch('GenNu2_pt', aGenNu2[0], 'aGenNu2_pt[nGenEvent]/F')
                  tree.Branch('GenNu3_pt', aGenNu3[0], 'aGenNu3_pt[nGenEvent]/F')
                  tree.Branch('GenNu4_pt', aGenNu4[0], 'aGenNu4_pt[nGenEvent]/F')
                  tree.Branch('GenNu1_eta', aGenNu1[1], 'aGenNu1_eta[nGenEvent]/F')
                  tree.Branch('GenNu2_eta', aGenNu2[1], 'aGenNu2_eta[nGenEvent]/F')
                  tree.Branch('GenNu3_eta', aGenNu3[1], 'aGenNu3_eta[nGenEvent]/F')
                  tree.Branch('GenNu4_eta', aGenNu4[1], 'aGenNu4_eta[nGenEvent]/F')
                  tree.Branch('GenNu1_phi', aGenNu1[2], 'aGenNu1_phi[nGenEvent]/F')
                  tree.Branch('GenNu2_phi', aGenNu2[2], 'aGenNu2_phi[nGenEvent]/F')
                  tree.Branch('GenNu3_phi', aGenNu3[2], 'aGenNu3_phi[nGenEvent]/F')
                  tree.Branch('GenNu4_phi', aGenNu4[2], 'aGenNu4_phi[nGenEvent]/F')
                  tree.Branch('GenNu1_mass', aGenNu1[3], 'aGenNu1_mass[nGenEvent]/F')
                  tree.Branch('GenNu2_mass', aGenNu2[3], 'aGenNu2_mass[nGenEvent]/F')
                  tree.Branch('GenNu3_mass', aGenNu3[3], 'aGenNu3_mass[nGenEvent]/F')
                  tree.Branch('GenNu4_mass', aGenNu4[3], 'aGenNu4_mass[nGenEvent]/F')
#				  GenNu1_pt  = []
#				  GenNu2_pt  = []
#				  GenNu3_pt  = []
#				  GenNu4_pt  = []
...
#				  GenNu3_mass= []
#				  GenNu4_mass= []
#                  for i in range(0, len(aGenNu1)):
#					  nGenEvent = i
#                      for j in range(len(aGenNu1[i])):
                  tree.Fill()
				  froot.Write()
                  fGenNu1 = open("./mets/GenNu1.txt", 'w')
                  for i in range(0, len(aGenNu1)):
                      for j in range(len(aGenNu1[i])):
                          fGenNu1.write(str(aGenNu1[i][j])+' ')
                      fGenNu1.write('\n')
                  fGenNu1.close()
                  fGenNu2 = open("./mets/GenNu2.txt", 'w')
                  for i in range(0, len(aGenNu2)):
                      for j in range(len(aGenNu2[i])):
                          fGenNu2.write(str(aGenNu2[i][j])+' ')
                      fGenNu2.write('\n')
                  fGenNu2.close()
                  fGenNu3 = open("./mets/GenNu3.txt", 'w')
                  for i in range(0, len(aGenNu3)):
                      for j in range(len(aGenNu3[i])):
                          fGenNu3.write(str(aGenNu3[i][j])+' ')
                      fGenNu3.write('\n')
                  fGenNu3.close()
                  fGenNu4 = open("./mets/GenNu4.txt", 'w')
                  for i in range(0, len(aGenNu4)):
                      for j in range(len(aGenNu4[i])):
                          fGenNu4.write(str(aGenNu4[i][j])+' ')
                      fGenNu4.write('\n')
                  fGenNu4.close()
                  #fGenMu1 = open("./mets/GenMu1.txt", 'w')
                  fGen = open("./mets/GenMET.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aGenMET)):
                      fGen.write(str(aGenMET[i])+'\n')
                  fGen.close()
                  fGenphi = open("./mets/GenMETphi.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aGenMETphi)):
                      fGenphi.write(str(aGenMETphi[i])+'\n')
                  fGenphi.close()
#              else:
#                  aMET, aMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, None)
              if obj=="L1PF":
                  aPFMET, aPFMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, None)
                  fPF = open("./mets/PFMET.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aPFMET)):
                      fPF.write(str(aPFMET[i])+'\n')
                  fPF.close()
                  fPFphi = open("./mets/PFMETphi.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aPFMETphi)):
                      fPFphi.write(str(aPFMETphi[i])+'\n')
                  fPFphi.close()
              if obj=="L1Puppi":
                  aPuppiMET, aPuppiMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, None)
                  fPu = open("./mets/PuMET.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aPuppiMET)):
                      fPu.write(str(aPuppiMET[i])+'\n')
                  fPu.close()
                  fPuphi = open("./mets/PuMETphi.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aPuppiMETphi)):
                      fPuphi.write(str(aPuppiMETphi[i])+'\n')
                  fPuphi.close()
              #print(obj + " MET")
              ...
              if options.var.startswith("met"):
                  ...
                  print(jecs)
              aCorrMET, aCorrMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, jecs)
              if obj=="L1PF":
                  aCorrPFMET, aCorrPFMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, jecs)
                  fCoPF = open("./mets/CoPFMET.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aCorrPFMET)):
                      fCoPF.write(str(aCorrPFMET[i])+'\n')
                  fCoPF.close()
                  fCoPFphi = open("./mets/CoPFMETphi.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aCorrPFMETphi)):
                      fCoPFphi.write(str(aCorrPFMETphi[i])+'\n')
                  fCoPFphi.close()
              if obj=="L1Puppi":
                  aCorrPuppiMET, aCorrPuppiMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, jecs)
                  fCoPu = open("./mets/CoPuMET.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aCorrPuppiMET)):
                      fCoPu.write(str(aCorrPuppiMET[i])+'\n')
                  fCoPu.close()
                  fCoPuphi = open("./mets/CoPuMETphi.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aCorrPuppiMETphi)):
                      fCoPuphi.write(str(aCorrPuppiMETphi[i])+'\n')
                  fCoPuphi.close()
              print(obj + " Corr MET")
              label = name
'''
...
hGenPF = ROOT.TH1F("hGenPF", "PF MET / Gen MET", 100, -5, 5)
hGenPu = ROOT.TH1F("hGenPu", "Puppi MET / Gen MET", 100, -5, 5)
hCoGenPF = ROOT.TH1F("hCoGenPF", "Corr PF MET / Gen MET", 100, -5, 5)
hCoGenPu = ROOT.TH1F("hCoGenPu", "Corr PF MET / Gen MET", 100, -5, 5)
for i in len(aGenMET):
              hGenPF.Fill(aPFMET[i]/aGenMET[i])
              hGenPu.Fill(aPuppiMET[i]/aGenMET[i])
              hCoGenPF.Fill(aCorrPFMET[i]/aGenMET[i])
              hCoGenPu.Fill(aCorrPuppiMET[i]/aGenMET[i])
