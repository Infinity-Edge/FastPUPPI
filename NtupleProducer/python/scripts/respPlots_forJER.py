...
def doRespPt(oname, tree, name, expr, cut, mcpt="mc_pt", xpt="mc_pt", fitopt="WQ0C EX0 ROB=0.95", fitrange=(0,999), maxEntries=999999999, respCorr="simple", fitfunc="lin",ptbins=None,keepGraph=False,zeroIsOk=True,fromGraph=None):
    ...
    for ipt,ptmax in enumerate(ptbins):
        ...
        if median > 0.2 or zeroIsOk:
            ...
            relres.append((ptc,ptd,(hi-lo)/(2.0*avg),0))
        if avg >  0.2 or zeroIsOk:
            resolsg.append((ptc,ptd,0.5*rms2,0))
            relresg.append((ptc,ptd,0.5*rms2/avg,0))
    #print "Got ",ret.GetN()
    ...
    if respCorr in ("gen","exact","fit"):
        ...
        for ipt,ptmax in enumerate(ptbins):
            ...
            if median > 0.2:
                resols.append((ptc,ptd,(hi-lo)/2,0))
                relres.append((ptc,ptd,(hi-lo)/(2,0*avg)))
                #print "for %s %s at pt ~ %6.1f  reco pt ~ %6.1f  [ %6.1f , %6.1f ]" % (oname,name, ptc, median, lo, hi)
            if avg >  0.2:
                resolsg.append((ptc,ptd,0.5*rms2,0))
                relresg.append((ptc,ptd,0.5*rms2/avg,0))
    ...
    relresol = ROOT.TGraphAsymmErrors(len(relres))
...
#        resol.SetPoint(ipoint, ptc, res/scale)
    ...
    if len(resolsg) > 3:
        ...
        relresolg = ROOT.TGraphAsymmErrors(len(relresg))
        resolg.SetName(name+"_gauss_res")
        relresolg.SetName(name+"_gauss_relres")
#        print(resolsg)
...
#            print(res)
            ...
            print("scale")
            print(scale)
            resolg.SetPoint(ipoint, ptc, res)
            resolg.SetPointError(ipoint, ptd, ptd, 0, 0)
        for ipoint,(ptc,ptd,res,err) in enumerate(relresg):
            if respCorr == "divide": scale = ret.Eval(ptc)
#            resolg.SetPoint(ipoint, ptc, res/scale)
#            print(res)
            print(ptd)
            print("scale")
            print(scale)
            relresolg.SetPoint(ipoint, ptc, res)
            relresolg.SetPointError(ipoint, ptd, ptd, 0, 0)
        resol.gaus = resolg
        rtf1g = rtf1.Clone(name+"_rf1g")
        resolg.Fit(rtf1g, fitopt+"W", "", fitrange[0], fitrange[1])
        resolg.fit = rtf1g
    return (retg,resolg,relresolg)
#    return (ret,resol)
