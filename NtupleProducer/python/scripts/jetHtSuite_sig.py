...
def makeGenArray(tree, what, ptCut, etaCut, metCut, sigCut, _cache={}):
    ...
    return ret
def makeGenMETArray(tree, what, etaCut, metCut, sigCut):
    if   etaCut <= 1.5: post = "MetBarrel_pt" 
    elif etaCut <= 2.4: post = "MetCentral_pt"
    else:               post = "Met_pt"
    if   etaCut <= 1.5: post_sig = "MetBarrel_sig" 
    elif etaCut <= 2.4: post_sig = "MetCentral_sig"
    else:               post_sig = "PuppiMet_sig"
    ...
    progress.done("done, %d entries" % len(ret))
    return ret
def makeGenMETSigArray(tree, what, etaCut, metCut, sigCut):
    if   etaCut <= 1.5: post_met = "MetBarrel_pt" 
    elif etaCut <= 2.4: post_met = "MetCentral_pt"
    else:               post_met = "Met_pt"
    if   etaCut <= 1.5: post = "MetBarrel_sig" 
    elif etaCut <= 2.4: post = "MetCentral_sig"
    else:               post = "PuppiMet_sig"
    progress = _progress("Reading gen"+post+" ...")
    progress_pu = _progress("Reading L1"+post+" ...")
    progress_met = _progress("Reading "+"L1Puppi"+post_met+" ...")
    tree.SetBranchStatus("*",0);
    tree.SetBranchStatus("gen"+post,1);
    tree.SetBranchStatus("L1Puppi"+post_met,1);
    tree.SetBranchStatus("L1"+post,1);
    ret = []
    for i in xrange(tree.GetEntries()):
        tree.GetEntry(i)
        #print(getattr(tree,"gen"+post_met))
        #print((getattr(tree,"gen"+post_met) >= metCut))
        if getattr(tree,"L1Puppi"+post_met) >= metCut and getattr(tree,"L1"+post) >= sigCut:
            ret.append(getattr(tree,"gen"+post))
    progress.done("done, %d entries" % len(ret))
    return ret
def makeCorrArray(tree, what, obj, ptCorrCut, etaCut, metCut, sigCut, corr, _cache={}):
    _key = (id(tree),what,obj,int(ptCorrCut*100),int(etaCut*1000))
    if _key in _cache: return _cache[_key]
    if what == "metmht":
        met = makeCorrArray(tree, "met", obj, ptCorrCut,    5.0, 0, corr, _cache=_cache)
        mht = makeCorrArray(tree, "mht", obj, ptCorrCut, etaCut, 0, corr, _cache=_cache)
        ret = map(min, zip(met,mht))
        _cache[_key] = ret
        return ret
    if "met" in what:
    ...
    progress.done("done, %d entries" % len(ret))
    return ret
def makeRecoMETArray(tree, what, obj, etaCut, metCut, sigCut):
    if   etaCut <= 1.5: post = "MetBarrel_pt" 
    elif etaCut <= 2.4: post = "MetCentral_pt"
    else:               post = "Met_pt"
    if   etaCut <= 1.5: post_sig = "MetBarrel_sig" 
    elif etaCut <= 2.4: post_sig = "MetCentral_sig"
    else:               post_sig = "Met_sig"
    if not tree.GetBranch(obj+post): 
        return None
    progress = _progress("Reading "+obj+post+" ...")
    progress_sig = _progress("Reading "+obj+post_sig+" ...")
    tree.SetBranchStatus("*",0);
    tree.SetBranchStatus(obj+post,1);
    tree.SetBranchStatus(obj+post_sig,1);
    ret = []
    for i in xrange(tree.GetEntries()):
        tree.GetEntry(i)
        if getattr(tree,obj+post) >= metCut and getattr(tree,obj+post_sig) >= sigCut:
            ret.append(getattr(tree,obj+post))
    progress.done("done, %d entries" % len(ret))
    return ret
def makeMETSigArray(tree, what, obj, etaCut, metCut, sigCut):
    if   etaCut <= 1.5: post_met = "MetBarrel_pt" 
    elif etaCut <= 2.4: post_met = "MetCentral_pt"
    else:               post_met = "Met_pt"
    if   etaCut <= 1.5: post = "MetBarrel_sig" 
    ...
    if not tree.GetBranch(obj+post): 
        return None
    progress = _progress("Reading "+obj+post+" ...")
    progress_met = _progress("Reading "+obj+post_met+" ...")
    tree.SetBranchStatus("*",0);
    tree.SetBranchStatus(obj+post,1);
    tree.SetBranchStatus(obj+post_met,1);
    ret = []
    for i in xrange(tree.GetEntries()):
        tree.GetEntry(i)
        if getattr(tree,obj+post_met) >= metCut and getattr(tree,obj+post) >= sigCut:
            ret.append(getattr(tree,obj+post))
    progress.done("done, %d entries" % len(ret))
    return ret
...
parser.add_option("-G", "--gmet", dest="gmet",  default=0., type="float", help="Choose gen MET cut")
parser.add_option("-M", "--gsig", dest="gsig",  default=0., type="float", help="Choose gen MET significance cut")
...
if options.var == "ht":
    ...
    qualif = "|#eta| < %.1f" % options.eta
elif options.var.startswith("sig"):
    if options.varlabel is None: options.varlabel = "E_{T}^{miss} Significance"
    if options.genht    is None: options.genht    = 0.
    if options.xmax     is None: options.xmax     = 20
    what = "sig"
    options.eta = 5.0 
    if "Central" in options.var:  options.eta = 2.4 
    elif "Barrel" in options.var: options.eta = 1.5
...
for plotkind in options.plots.split(","):
  ...
  if plotkind != "rate":
    genArray = makeGenArray(signal, what, options.pt, options.eta, options.gmet, options.gsig)
  ...
  for targetrate in rates:
      for objset,things in whats:
          ...
          for name,obj,col,msty,msiz in things:
              ...
              label = name
              if plotkind == "rate":
                  recoArrayB = makeCorrArray(background, what, obj, options.pt, options.eta, options.gmet, options.gsig, jecs)
                  if not recoArrayB: continue
                  plot = makeCumulativeHTEff(name, recoArrayB, options.xmax)
              elif plotkind == "effc":
                  recoArrayS = makeCorrArray(signal, what, obj, options.pt, options.eta, options.gmet, options.gsig, jecs)
