...
from FastPUPPI.NtupleProducer.scripts.respPlots_forJER import doRespPt
...
if __name__ == "__main__":
    ...
    #parser.add_option("--etabins", dest="etabins", default="0.,1.5")
    #parser.add_option("--etabins", dest="etabins", default="0.,2.4")
    parser.add_option("--etabins", dest="etabins", default="0.,5.")
    #parser.add_option("--etabins", dest="etabins", default="0,1.3,1.7,2.1,2.4,2.8,3.2,4.0")
    ...
    parser.add_option("--dr", dest="drname", default="VBF_PU200")
    options, args = parser.parse_args()

    tree = ROOT.TChain("Events")
    os.system("mkdir "+options.drname)
    filenames = []
    for a in args[:]:
        if a.endswith(".root"): 
            filenames.append(a)
    ...
    if options.auto:
        for b in tree.GetListOfBranches():
            ...
            if bname.endswith("GenJets_pt"):
                whats.append(bname.replace("_pt",""))
                print "Got branch "+whats[-1]
    ...
    for w in whats:
        ...
        for ieta in xrange(1,len(etas)):
            ...
            if w == "GenJets": cut = "{mcpt} > 20 && {etamin} < abs({w}_eta) && abs({w}_eta) < {etamax}".format(w=w, mcpt=mcpt, etamin=etas[ieta-1],etamax=etas[ieta])
            perf = _progress( "Making response for %s (%.3f < |eta| < %.3f)..." % (w,etas[ieta-1],etas[ieta]))
            prof, resol = doRespPt("jet",tree,w,expr,cut,mcpt=mcpt,xpt=mcpt,fitrange=options.fitrange,fitfunc="none",ptbins=ptForResp,keepGraph=options.closurePlots)
#            prof, resol, relres = doRespPt("jet",tree,w,expr,cut,mcpt=mcpt,xpt=mcpt,fitrange=options.fitrange,fitfunc="gaus",ptbins=ptForResp,keepGraph=options.closurePlots)
            perf.done()
            ...
            if len([r for r in responses if r > 0.2]) < 3:
                ...
                continue
            tfnew = ROOT.TFile.Open(options.drname+"/"+w+".root", "RECREATE")
            for m in methods:
                h = ptCorrs[(m,w)]
                if m in ("fitlin",):
                    respformula = "1/x++1"
