...
def makeCorrMetArray(tree, obj, ptCorrCut, etaCut, corr, _cache={}):
    ...
    for i in xrange(tree.GetEntries()):
        ...
        rawpt,eta,phi = getattr(tree, obj+"Jets_pt"), getattr(tree, obj+"Jets_eta"), getattr(tree, obj+"Jets_phi")
        jets = [ ]
        if (obj == "L1TK" or obj == "L1TKV" or obj == "L1TKV6") and eta > 2.8: continue
...
parser.add_option("-P","--plots", dest="plots", default="rate", help="Choose plot or comma-separated list of plots")
  ...
  for targetrate in rates:
      for objset,things in whats:
          ...
          for name,obj,col,msty,msiz in things:
              ...
              obj = obj.replace("$","")
              if obj=="Gen":
#                  aMET, aMETphi = makeGenMETArray(signal, obj, options.eta)
                  aGenMET, aGenMETphi = makeGenMETArray(signal, obj, options.eta)
                  fGen = open("./mets/"+pu+"/GenMET_"+pu+".txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aGenMET)):
                      fGen.write(str(aGenMET[i])+'\n')
                  fGen.close()
                  fGenphi = open("./mets/"+pu+"/GenMETphi_"+pu+".txt", 'w')
                  #for i in range(0, 1000):
...
#                  aMET, aMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, None)
              if obj!="Gen":
                  aMET, aMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, None)
                  fMET = open("./mets/"+pu+"/"+obj+"MET_"+pu+".txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aMET)):
                      fMET.write(str(aMET[i])+'\n')
                  fMET.close()
                  fMETphi = open("./mets/"+pu+"/"+obj+"METphi_"+pu+".txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aMETphi)):
                      fMETphi.write(str(aMETphi[i])+'\n')
                  fMETphi.close()
                  print(obj + " MET")
              ...
              if obj!="Gen":
                  aCoMET, aCoMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, jecs)
                  fCoMET = open("./mets/"+pu+"/Co"+obj+"MET_"+pu+".txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aCoMET)):
                      fCoMET.write(str(aCoMET[i])+'\n')
                  fCoMET.close()
                  fCoMETphi = open("./mets/"+pu+"/Co"+obj+"METphi_"+pu+".txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aCoMETphi)):
                      fCoMETphi.write(str(aCoMETphi[i])+'\n')
                  fCoMETphi.close()
                  print(obj + " Co MET")
              label = name
              coun+=1
